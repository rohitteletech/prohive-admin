create or replace function public.guard_attendance_policy_active_schedule()
returns trigger
language plpgsql
...
drop trigger if exists guard_attendance_policy_active_schedule on public.company_policy_definitions;

create trigger guard_attendance_policy_active_schedule
before insert or update of policy_type, status, effective_from
on public.company_policy_definitions
for each row
when (new.policy_type = 'attendance' and new.status = 'active')
execute function public.guard_attendance_policy_active_schedule();

create or replace function public.save_attendance_policy_definition(
  p_company_id uuid,
  p_admin_email text,
  p_policy_id uuid,
  p_policy_name text,
