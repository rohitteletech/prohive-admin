...
export default function Page() {
  ...
  function addShift() {
    const template = rows[0] || DEFAULT_COMPANY_SHIFTS[0];
    const id = `s${Date.now()}`;
    const next: ShiftRow = {
      id,
      name: "New Shift",
      ...
      graceMins: template?.graceMins ?? 10,
      earlyWindowMins: template?.earlyWindowMins ?? 15,
      minWorkBeforeOutMins: template?.minWorkBeforeOutMins ?? 60,
      active: true,
    };
